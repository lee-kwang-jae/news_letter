...
<!-- 지역 뉴스 기사 4 (신아일보: 하남시의회 임시회 폐회…1조1500억원 규모 추경 의결) -->
<div class="article-card">
<div class="badge">📰 지방의회/행정</div>
<h3><a href="https://www.shinailbo.co.kr/news/articleView.html?idxno=5063434" target="_blank" style="color: inherit; text-decoration: none;">하남시의회 제352회 임시회 폐회… 1조 1500억 원 규모 추경안 및 17개 안건 의결</a></h3>
<div class="summary" style="text-align: left; word-break: keep-all; letter-spacing: -0.3px;">
하남시의회가 제352회 임시회 제2차 본회의를 열고 1조 1,500억 원 규모의 2026년도 제2회 추가경정예산안 등 총 17건의 안건을 최종 의결했습니다. 예산결산특별위원회 심사를 거쳐 28억여 원이 조정·삭감되었으며, 본회의에서는 최승태 의원의 '지역 간 균형 있는 안전정책 수립 촉구' 및 오승철 의원의 '캠프콜번 개발 방향' 등 의원 5명의 5분 자유발언이 진행됐습니다.
<div style="margin-top: 12px; font-size: 0.9em; color: #718096;"><a href="https://www.shinailbo.co.kr/news/articleView.html?idxno=5063434" target="_blank" style="color: #3182ce; font-weight: bold; text-decoration: none;">기사 원문 보기 (신아일보) →</a></div>
</div>
<div class="source">
📌 출처: 신아일보 (김두평 기자)
</div>
...
for filepath in target_files:
    if os.path.exists(filepath):
        ...
        if idx_local != -1 and idx_mom != -1:
            idx_comment = content.find('<!-- ===== 섹션 2: 하남 지역 주요 뉴스 ===== -->')
            start_pos = idx_comment if (idx_comment != -1 and idx_comment < idx_local) else idx_local
            idx_hr_after = content.find('<hr/>', idx_local)
            if idx_hr_after != -1 and idx_hr_after <= idx_mom:
                updated_content = content[:start_pos] + new_section2_content + '\n' + content[idx_hr_after:]
                with open(filepath, 'w', encoding='utf-8') as f:
                    f.write(updated_content)
                print(f"Successfully updated Section 2 in {filepath}")
            else:
                print(f"Failed to find hr positions in {filepath}")
        else:
